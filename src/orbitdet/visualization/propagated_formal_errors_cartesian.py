...
class PropagatedFormalErrorsCartesian(Plot):
    """Plot propagated formal errors in Cartesian coordinates (x, y, z).
    ...
    formal_errors : np.ndarray
        Shape ``(N, >=3)`` array of formal errors; columns 0, 1, 2 = x, y, z.
    ...
    def __init__(
        ...
        super().__init__(cfg)
        self.epochs = epochs
        self.formal_errors = formal_errors
        self.observation_windows = observation_windows

    def _make_figure(self):
        cfg = self.cfg
        epochs = self.epochs
        formal_errors = self.formal_errors
        observation_windows = self.observation_windows

        plot_cfg = _cfg_get(cfg, "propagated_formal_errors_cartesian", default=None)
        ...
        ax.plot(times, formal_errors[:, 0], label=r"$x$", alpha=0.8)
        ax.plot(times, formal_errors[:, 1], label=r"$y$", alpha=0.6)
        ax.plot(times, formal_errors[:, 2], label=r"$z$", alpha=0.6)
        ...
        title = _cfg_get(
            plot_cfg, "titles", "title", default="Propagated Formal Errors - Cartesian"
        )
